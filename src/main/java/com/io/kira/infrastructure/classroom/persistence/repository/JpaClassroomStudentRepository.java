...
public interface JpaClassroomStudentRepository extends JpaRepository<ClassroomStudentEntity, Long> {
    ...
    @Query("""
            SELECT new com.io.kira.application.activity.result.StudentSummaryData(
                cs.studentUserId,
                u.firstName,
                u.lastName,
                u.profileUrl
            ...
            WHERE cs.classroom.classroomId = :classroomId
              AND cs.status = com.io.kira.domain.classroom.valueObject.StudentStatus.ACTIVE
    ...
    @Query("""
            SELECT new com.io.kira.application.classroom.result.ClassroomStudentJoinedData(
                cs.studentUserId,
                u.firstName,
                u.lastName,
                u.profileUrl,
    ...
    SELECT COUNT(cs)
    FROM ClassroomStudentEntity cs
    WHERE cs.status = com.io.kira.domain.classroom.valueObject.StudentStatus.ACTIVE
