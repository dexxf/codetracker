...
public class AddActivityService {
    ...
    public ActivityResponse execute(AddActivityCommand command) {
        ...
        if(!activityCreationRes.success())
            return ActivityResponse.fail(activityCreationRes.error().getMessage());

        var saveRes = activityAppRepository.save(activityCreationRes.data());

        if(saveRes == null) return ActivityResponse.fail("Saving failed.");

        return ActivityResponse.success(saveRes, "Successfully added activity");
    }
