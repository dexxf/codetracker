...
public final class AuthRegistration {
    ...
    public AuthRegistrationDTO execute(AuthRegistrationCommand command) {
        if(authAppRepository.emailExists(command.email())) {
            return AuthRegistrationDTO.fail("Email taken.");
        }
        ...
        if(!authCreationResult.success()) {
            return AuthRegistrationDTO.fail(authCreationResult.error().getMessage());
        }
        ...
        return AuthRegistrationDTO.success(auth);
    }
