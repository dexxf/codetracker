...
public final class UserProfileService {
       ...
       public UserProfileDTO execute(UserProfileCommand command) {
           Optional<User> userOpt = repository.findByUserId(command.userId());

           if(userOpt.isEmpty())
               return UserProfileDTO.fail("User not found.");
           ...
           if(userProfileUpdaterResult.hasErrors())
               return UserProfileDTO.fail(userProfileUpdaterResult.errors());

           repository.save(user);
           return UserProfileDTO.ok(user);
       }
}
