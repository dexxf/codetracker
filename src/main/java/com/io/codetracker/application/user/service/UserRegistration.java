...
public final class UserRegistration {
    ...
    public UserRegistrationDTO completeInitialization(UserRegistrationCommand command) {
        Optional<User> userOpt = repository.findByUserId(command.userId());

        if (userOpt.isEmpty()) return UserRegistrationDTO.fail("User not found.");

        User user = userOpt.get();

        if(user.isHasFullyInitialized()) return UserRegistrationDTO.fail("User already fully initialized.");

        Gender gender;
        try {
            gender = Gender.valueOf(command.gender().toUpperCase());
        } catch (IllegalArgumentException e) {
            return UserRegistrationDTO.fail("Invalid gender.");
        }
        ...
        if (userFinalizeResult != UserCreationResult.SUCCESS) {
            return UserRegistrationDTO.fail(userFinalizeResult.getMessage());
        }
        ...
        return UserRegistrationDTO.ok(data, "Profile completed.");
    }
